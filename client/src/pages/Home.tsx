...
const modules: Module[] = [
  {
    ...
    description: "Use esta tela durante o turno: encontre quem chegou, registre a entrada, marque uma falta quando necessário e encerre o trabalho no final.",
    tone: "blue",
    icon: LayoutDashboard,
    route: "/",
    duration: "3 min",
    steps: ["No início, procure o trabalhador em Vagas de hoje e clique em Check-in.", "Confira a foto e clique em Confirmar check-in.", "No final, use Avaliar e depois Forçar encerramento para fechar o turno."],
    tip: "Antes de confirmar o check-in, confira a foto do trabalhador. Se houver dúvida, peça um documento ou confirme o agendamento.",
    details: [
      { title: "Se a pessoa não chegar", body: "Aguarde o tempo de tolerância combinado, normalmente 30 minutos. Depois, clique em Falta e confirme. Essa ação pode bloquear o trabalhador e cancelar agendamentos futuros por 15 dias." },
  ...
  return (
    <div className="real-screen-wrap">
      ...
      <div className="real-screen platform-capture">
        <aside className="capture-sidebar"><strong>iWof</strong><span>⌂ Dashboard</span><span className={isJobs ? "capture-active" : ""}>▣ Vagas</span><span className={isBookings ? "capture-active" : ""}>▤ Agendamentos</span><span className={isFinance ? "capture-active" : ""}>▧ Financeiro</span><i /><span>▱ iWof Loja 1　›</span><span className={isAdmin ? "capture-active" : ""}>⚙ Admin</span><span className={isProfile ? "capture-active" : ""}>♙ Perfil</span></aside>
        <div className="capture-main"><div className="capture-top"><span>Hoje, 11 de setembro de 2026</span><b>BR</b></div><div className="capture-body"><div className="capture-title"><div><h4>{title}</h4><p>{isJobs ? "Gerencie todas as vagas da loja iWof Loja 1." : isBookings ? "Consulte e gerencie os agendamentos da loja iWof Loja 1." : isFinance ? "Detalhamento da cobrança da loja iWof Loja 1 referente ao período selecionado." : isAdmin ? "Gerencie os usuários da sua empresa." : isProfile ? "Gerencie suas informações pessoais e configurações da conta." : "Acompanhe as vagas e a presença da sua equipe."}</p></div>{isJobs && <b className="capture-add">＋ Adicionar vaga</b>}</div>
          {isAdmin ? <div className="capture-admin-tabs"><b>Funções</b><b className="tab-selected">Usuários</b><div className="capture-search">⌕　Buscar por nome ou e-mail</div></div> : isProfile ? <div className="capture-profile"><div><small>INFORMAÇÕES PESSOAIS</small><strong>Nome completo<br /><em>Breno</em></strong><strong>E-mail<br /><em>breno@iwof.com.br</em></strong></div><div><small>SEGURANÇA</small><strong>Senha　••••••••••</strong><b className="capture-outline">Alterar senha</b></div></div> : <><div className="capture-filters"><span>{isJobs ? "⌕ Buscar por id" : "Início　 01/09/2026"}</span><span>{isJobs ? "Todas as funções" : isFinance ? "Fim　 11/09/2026" : "Fim　 26/09/2026"}</span><span>{isJobs ? "Horário da escala" : isBookings ? "Todas　 Função" : "Todas as funções"}</span></div><div className="capture-cards"><b><small>{isFinance ? "Valor Total do Período" : isBookings ? "Total de Agendamentos" : "Vagas disponíveis"}</small><strong>{isFinance ? "R$ 0,00" : isBookings ? "0" : "2.486"}</strong></b><b><small>{isFinance ? "Horas Trabalhadas" : isBookings ? "Em andamento" : "Vagas ocupadas"}</small><strong>{isFinance ? "0 h" : "0"}</strong></b><b><small>{isFinance ? "Funções Faturadas" : isBookings ? "Realizados" : "Total de vagas"}</small><strong>0</strong></b></div><div className="capture-table"><span>{isJobs ? "Vaga　 Sexo / Certificado　 Período　 Escalas　 Quantidade" : isBookings ? "Nome　 Função　 Data　 Horário　 Status" : isFinance ? "Nenhum agendamento concretizado no período selecionado." : "Resumo da operação da loja"}</span><i /><i /><i /><i /></div></>}
        </div></div>
      </div>
